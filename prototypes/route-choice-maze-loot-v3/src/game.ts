...
import { Backpack, InvItem, WeaponDef, makeWeaponItem, makeTreasureItem, makeConsumableItem, makeAmmoItem, TreasureItem, CONSUMABLE_INFO, ConsumableKind, RARITY_INFO, RELICS, rollCrateContent, rollLowQualityTreasure } from './items';
...
export class Game {
  ...
  private tryAddItem(item: InvItem): void {
    ...
    if (this.bag.addItem(item)) {
      ...
      if (item.kind === 'treasure' && this.greedRoomActive) {
      ...
      if (item.kind === 'treasure' && this.luckyStarLayers > 0) {
  ...
  private addConfiguredBonusItem(item: InvItem, reason: string): void {
    if (item.kind === 'treasure') this.meta.collect(item.name);
    if (this.bag.addItem(item)) {
      this.hud.logPickup(item.icon, `${item.name}（${reason}）`, 1, item.color);
    } else {
      this.hud.showToast(`🎒 背包空间不足，${reason}的 ${item.name} 未能放入`);
    }
  }
