...
export class PlayerRig {
  ...
  takeMonsterDamage(amount: number): { hpLost: number; shieldLost: number } {
    ...
    const temporaryShieldBefore = this.temporaryShield;
    let dmg = amount * this.dmgTakenMult;
    if (this.temporaryShield > 0) {
      const blocked = Math.min(this.temporaryShield, dmg);
      this.temporaryShield -= blocked;
      dmg -= blocked;
    }
    if (this.shield > 0) {
    ...
    return {
      ...
      shieldLost: Math.max(0, shieldBefore - this.shieldCount) + (temporaryShieldBefore > this.temporaryShield ? 1 : 0),
    };
  }
  ...
  addTemporaryShield(amount: number, cap: number = 10): number {
    const before = this.temporaryShield;
    this.temporaryShield = Math.min(cap, this.temporaryShield + amount);
    return this.temporaryShield - before;
  }
