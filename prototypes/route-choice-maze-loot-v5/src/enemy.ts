...
const KIND_CONF: Record<ZombieKind, KindConf> = {
  normal:  { name: '食尸鬼',   baseHp: 30,  speedMult: 1,    scale: 1,    hue: 0.27, bodyArmor: 1,    touchMult: 1,   ranged: false, psychic: false, opacity: 1 },
  fast:    { name: '开膛手',   baseHp: 20,  speedMult: 2.1,  scale: 0.85, hue: 0.09, bodyArmor: 1,    touchMult: 0.8, ranged: false, psychic: false, opacity: 1 },
  ranged:  { name: '追踪者',   baseHp: 24,  speedMult: 0.8,  scale: 1,    hue: 0.55, bodyArmor: 1,    touchMult: 0.8, ranged: true,  psychic: false, opacity: 1 },
  heavy:   { name: '再生者',   baseHp: 72,  speedMult: 0.55, scale: 1.35, hue: 0.0,  bodyArmor: 0.4,  touchMult: 1.8, ranged: false, psychic: false, opacity: 1 },
  psychic: { name: '无形之子', baseHp: 22,  speedMult: 0.9,  scale: 1.05, hue: 0.72, bodyArmor: 1,    touchMult: 0,   ranged: false, psychic: true,  opacity: 0.45 },
  elite:   { name: '巨人',     baseHp: 140, speedMult: 0.7,  scale: 1.9,  hue: 0.83, bodyArmor: 0.7,  touchMult: 2.4, ranged: false, psychic: false, opacity: 1 },
};

const CHAPTER_HP_MULT = [1.0, 1.45, 2.05];
...
export class Zombie {
  ...
  constructor(scene: THREE.Scene, pos: THREE.Vector3, depth: number, kind: ZombieKind) {
    ...
    this.conf = c;
    const chapterIndex = Math.max(0, Math.min(CHAPTER_HP_MULT.length - 1, Math.floor(Math.max(0, depth - 1) / 30)));
    const depthInChapter = ((Math.max(1, depth) - 1) % 30) + 1;
    const chapterMult = CHAPTER_HP_MULT[chapterIndex];
    const depthMult = 1 + depthInChapter * 0.025;
    this.maxHp = Math.round(c.baseHp * chapterMult * depthMult);
    this.hp = this.maxHp;
...
export class ZombieManager {
  ...
  composition(depth: number, count: number): ZombieKind[] {
    ...
    const chapter = Math.max(1, Math.ceil(depth / 30));
    for (let i = 0; i < count; i++) {
      const r = Math.random();
      if (chapter >= 3) {
        if (r < 0.20) out.push('psychic');
        else if (r < 0.43) out.push('heavy');
        else if (r < 0.72) out.push('ranged');
        else if (r < 0.88) out.push('fast');
        else out.push('normal');
      } else if (chapter === 2) {
        if (r < 0.16) out.push('psychic');
        else if (r < 0.34) out.push('heavy');
        else if (r < 0.62) out.push('ranged');
        else if (r < 0.82) out.push('fast');
        else out.push('normal');
      } else if (depth >= 8 && r < 0.12) out.push('psychic');
