...
import { Backpack, BackpackSnapshot, DragPlacement, InventoryPlacementTransaction, InvItem, RARITY_INFO, ConsumableKind, CONSUMABLE_INFO } from './items';
...
interface InventoryDragState {
  ...
  ghost: HTMLElement;
  el: HTMLElement;
  transaction: InventoryPlacementTransaction;
  source: 'grid' | 'pending';
  preview: DragPlacement | null;
  pointerId: number;
  grabCellX: number;
  grabCellY: number;
  offX: number;
...
interface PendingDiscardRecord {
  item: InvItem;
  snapshot: BackpackSnapshot;
}

interface PendingLootSession {
  item: InvItem;
  reason: string;
  initialSnapshot: BackpackSnapshot;
  discards: PendingDiscardRecord[];
  onConfirm: (discarded: InvItem[]) => void;
  onAbandon: () => void;
}
...
export class HUD {
  ...
  private drag: InventoryDragState | null = null;
  private pendingLoot: PendingLootSession | null = null;
  private pendingDiscard: HTMLElement;
  private inventoryCell: number = 44;
  ...
  constructor() {
    ...
    this.pendingDiscard = $('pending-discard');
    ...
    $('bag-sort').addEventListener('click', () => {
      if (!this.bag) return;
      this.cancelActiveDrag(false);
      this.closePopup();
      const arranged = this.bag.autoArrange();
      this.showToast(arranged ? '背包整理完成' : '当前物品无法完成装箱');
      this.renderInventory();
    });
    $('pending-undo').addEventListener('click', () => this.undoPendingDiscard());
    $('pending-confirm').addEventListener('click', () => this.confirmPendingLoot());
    $('pending-abandon').addEventListener('click', () => this.abandonPendingLoot());
  ...
  private onBagKeyDown(e: KeyboardEvent): void {
    if (this.pendingLoot && e.key === 'Escape') {
      e.preventDefault();
      e.stopImmediatePropagation();
      this.closePopup();
      this.showToast('必须确认拾取或放弃新物品');
      return;
    }
    if (e.code === 'KeyR' && this.drag && !e.repeat && !e.ctrlKey && !e.altKey && !e.metaKey) {
      e.preventDefault();
      e.stopImmediatePropagation();
      this.rotateActiveDrag();
      return;
    }
    ...
    if (this.isOverlayVisible('start-overlay') || this.isOverlayVisible('end-overlay') ||
      this.isOverlayVisible('choice-overlay') ||
  ...
  renderInventory(): void {
    if (!this.bag) return;
    const pendingWidth = Math.min(494, Math.max(300, this.stage.clientWidth - 24));
    const pendingSide = this.stage.clientWidth <= 470 ? 124 : 190;
    const maxGridWidth = this.pendingLoot
      ? Math.max(190, pendingWidth - pendingSide - 24)
      : Math.min(352, Math.max(240, this.stage.clientWidth - 36));
    const maxGridHeight = this.pendingLoot
      ? 238
      : Math.min(308, Math.max(220, this.stage.clientHeight * 0.42));
    this.inventoryCell = Math.max(26, Math.floor(Math.min(
      44,
      maxGridWidth / this.bag.cols,
      maxGridHeight / this.bag.rows,
    )));
    const cell = this.inventoryCell;
    this.bagOverlay.style.setProperty('--cell', `${cell}px`);
    this.invGrid.style.width = `${this.bag.cols * cell}px`;
    this.invGrid.style.height = `${this.bag.rows * cell}px`;
    // 网格物品
    ...
    this.renderPendingLoot();
  }

  private attachItem(el: HTMLElement, item: InvItem): void {
    el.addEventListener('pointerdown', (e: PointerEvent) => {
      if (!this.bag || this.drag || (e.pointerType === 'mouse' && e.button !== 0)) return;
      e.preventDefault();
      this.closePopup();
      const transaction = this.bag.beginDragTransaction(item);
      if (!transaction) return;
      this.startItemDrag(el, item, transaction, 'grid', e);
    });
  }

  private attachPendingItem(el: HTMLElement, item: InvItem): void {
    el.addEventListener('pointerdown', (e: PointerEvent) => {
      if (!this.bag || this.drag || (e.pointerType === 'mouse' && e.button !== 0)) return;
      e.preventDefault();
      this.closePopup();
      const transaction = this.bag.beginInsertTransaction(item);
      if (!transaction) return;
      this.startItemDrag(el, item, transaction, 'pending', e);
    });
  }

  private startItemDrag(
    el: HTMLElement, item: InvItem, transaction: InventoryPlacementTransaction,
    source: 'grid' | 'pending', e: PointerEvent,
  ): void {
    const rect = el.getBoundingClientRect();
    const ghost = document.createElement('div');
    ghost.className = 'inv-ghost';
    ghost.style.width = `${transaction.width * this.inventoryCell - 4}px`;
    ghost.style.height = `${transaction.height * this.inventoryCell - 4}px`;
    ghost.style.borderColor = item.color;
    ghost.innerHTML = `<span>${item.icon}</span>`;
    document.body.appendChild(ghost);
    el.classList.add('drag-source');
    this.drag = {
      item, ghost, el, transaction, source, preview: null,
      pointerId: e.pointerId,
      grabCellX: Math.max(0, Math.min(transaction.width - 1,
        Math.floor((e.clientX - rect.left) / (rect.width / transaction.width)))),
      grabCellY: Math.max(0, Math.min(transaction.height - 1,
        Math.floor((e.clientY - rect.top) / (rect.height / transaction.height)))),
      offX: e.clientX - rect.left, offY: e.clientY - rect.top,
      downX: e.clientX, downY: e.clientY,
      pointerX: e.clientX, pointerY: e.clientY, moved: false,
    };
    this.updateDragPreview(e.clientX, e.clientY);
  }

  private clearDragPreview(): void {
    this.invGrid.querySelectorAll('.inv-drop-cell').forEach((cell) => cell.remove());
  }

  private renderDragPreview(placement: DragPlacement): void {
    this.clearDragPreview();
    const startX = placement.valid ? placement.cx : placement.requestedCx;
    const startY = placement.valid ? placement.cy : placement.requestedCy;
    for (let y = startY; y < startY + placement.h; y++) {
      for (let x = startX; x < startX + placement.w; x++) {
        if (!this.bag || x < 0 || y < 0 || x >= this.bag.cols || y >= this.bag.rows) continue;
        const cell = document.createElement('div');
        cell.className = `inv-drop-cell ${placement.valid ? 'valid' : 'invalid'}`;
        cell.style.left = `${x * this.inventoryCell}px`;
        cell.style.top = `${y * this.inventoryCell}px`;
  ...
  private updateDragPreview(x: number, y: number): void {
    ...
    d.pointerX = x;
    d.pointerY = y;
    const gridRect = this.invGrid.getBoundingClientRect();
    const overGrid = x >= gridRect.left && x < gridRect.right && y >= gridRect.top && y < gridRect.bottom;
    d.ghost.classList.remove('drop-valid', 'drop-invalid');
    if (!overGrid) {
      d.preview = null;
      this.clearDragPreview();
    ...
    const pointerCellX = Math.floor((x - gridRect.left) / this.inventoryCell);
    const pointerCellY = Math.floor((y - gridRect.top) / this.inventoryCell);
    const cx = pointerCellX - d.grabCellX;
    const cy = pointerCellY - d.grabCellY;
    const placement = d.transaction.preview(cx, cy);
    d.preview = placement;
    this.renderDragPreview(placement);
    d.ghost.style.width = `${placement.w * this.inventoryCell - 4}px`;
    d.ghost.style.height = `${placement.h * this.inventoryCell - 4}px`;
    d.ghost.classList.add(placement.valid ? 'drop-valid' : 'drop-invalid');
    const displayCx = placement.valid ? placement.cx : placement.requestedCx;
    const displayCy = placement.valid ? placement.cy : placement.requestedCy;
    d.ghost.style.left = `${gridRect.left + displayCx * this.inventoryCell + 1}px`;
    d.ghost.style.top = `${gridRect.top + displayCy * this.inventoryCell + 1}px`;
  }

  private rotateActiveDrag(): void {
    if (!this.drag) return;
    const d = this.drag;
    const oldGrabX = d.grabCellX;
    const oldGrabY = d.grabCellY;
    const oldHeight = d.transaction.height;
    if (!d.transaction.rotate()) return;
    d.grabCellX = oldHeight - 1 - oldGrabY;
    d.grabCellY = oldGrabX;
    d.ghost.style.width = `${d.transaction.width * this.inventoryCell - 4}px`;
    d.ghost.style.height = `${d.transaction.height * this.inventoryCell - 4}px`;
    d.moved = true;
    this.updateDragPreview(d.pointerX, d.pointerY);
  }
  ...
  private onPointerMove(e: PointerEvent): void {
    ...
    this.wslotEls.forEach((slotEl) => {
      ...
      slotEl.classList.toggle('drop-ok', !this.pendingLoot && over && this.drag!.item.kind === 'weapon');
    });
    const discardRect = this.pendingDiscard.getBoundingClientRect();
    const overDiscard = !!this.pendingLoot && this.drag.source === 'grid'
      && this.drag.item !== this.pendingLoot.item
      && e.clientX >= discardRect.left && e.clientX <= discardRect.right
      && e.clientY >= discardRect.top && e.clientY <= discardRect.bottom;
    this.pendingDiscard.classList.toggle('drop-ready', overDiscard);
  }

  private onPointerUp(e: PointerEvent): void {
    if (!this.drag || !this.bag || e.pointerId !== this.drag.pointerId) return;
    const d = this.drag;
    this.drag = null;

    if (!d.moved) {
      d.transaction.cancel();
      this.finishDragVisual(d, true);
      if (d.source === 'grid') this.showItemPopup(d.item, e.clientX, e.clientY);
      return;
    }
    ...
    if (this.pendingLoot && d.source === 'grid' && d.item !== this.pendingLoot.item) {
      const discardRect = this.pendingDiscard.getBoundingClientRect();
      const overDiscard = e.clientX >= discardRect.left && e.clientX <= discardRect.right
        && e.clientY >= discardRect.top && e.clientY <= discardRect.bottom;
      if (overDiscard) {
        d.transaction.cancel();
        this.finishDragVisual(d, true);
        this.stagePendingDiscard(d.item);
        return;
      }
    }

    // 普通背包模式可拖到武器栏；待处理模式只允许整理占格。
    if (!this.pendingLoot && d.source === 'grid') {
      for (let slot = 0; slot < this.wslotEls.length; slot++) {
        const r = this.wslotEls[slot].getBoundingClientRect();
        if (e.clientX >= r.left && e.clientX <= r.right && e.clientY >= r.top && e.clientY <= r.bottom) {
          d.transaction.cancel();
          const equipped = d.item.kind === 'weapon' && this.bag.equipToSlot(d.item.id, slot);
          if (equipped) {
            if (this.onInventoryDirty) this.onInventoryDirty();
            this.finishDragVisual(d, true);
            this.renderInventory();
          } else {
            this.finishDragVisual(d, false);
          }
          return;
        }
      }
    ...
    if (d.preview) {
      const result = d.transaction.commit(d.preview);
      if (result.committed) {
        this.finishDragVisual(d, true);
        this.renderInventory();
        return;
  ...
  private finishDragVisual(d: InventoryDragState, accepted: boolean): void {
    this.clearDragPreview();
    this.wslotEls.forEach((el) => el.classList.remove('drop-ok'));
    this.pendingDiscard.classList.remove('drop-ready');
    if (accepted) {
      d.ghost.remove();
    ...
    const sourceRect = d.el.getBoundingClientRect();
    d.ghost.classList.remove('drop-valid');
    d.ghost.classList.add('drop-invalid', 'returning');
    requestAnimationFrame(() => {
      d.ghost.style.left = `${sourceRect.left}px`;
      d.ghost.style.top = `${sourceRect.top}px`;
    ...
    d.ghost.addEventListener('transitionend', cleanup, { once: true });
    window.setTimeout(cleanup, 240);
  }

  private cancelActiveDrag(animate: boolean): void {
    if (!this.drag) return;
    const d = this.drag;
    this.drag = null;
    d.transaction.cancel();
    if (animate) this.finishDragVisual(d, false);
    else {
      this.clearDragPreview();
      this.wslotEls.forEach((el) => el.classList.remove('drop-ok'));
      this.pendingDiscard.classList.remove('drop-ready');
  ...
  showPendingLoot(
    item: InvItem, reason: string,
    onConfirm: (discarded: InvItem[]) => void,
    onAbandon: () => void,
  ): void {
    if (!this.bag || this.pendingLoot) return;
    this.closePopup();
    this.pendingLoot = {
      item,
      reason,
      initialSnapshot: this.bag.createSnapshot(),
      discards: [],
      onConfirm,
      onAbandon,
    };
    this.bagOverlay.classList.add('pending-mode');
    document.getElementById('bag-title')!.textContent = '待处理物品';
    this.bagOverlay.style.display = 'flex';
    if (this.onBagToggle) this.onBagToggle(true);
    this.renderInventory();
  }

  private renderPendingLoot(): void {
    if (!this.bag || !this.pendingLoot) return;
    const session = this.pendingLoot;
    const item = session.item;
    const placed = this.bag.items.includes(item);
    const panel = document.getElementById('pending-new-panel')!;
    panel.innerHTML = '';

    const heading = document.createElement('div');
    heading.className = 'pending-panel-title';
    heading.textContent = session.reason || '新发现物品';
    panel.appendChild(heading);

    const shape = document.createElement('div');
    shape.className = `pending-item-shape${placed ? ' placed' : ''}`;
    shape.style.width = `${Math.max(48, item.w * 36)}px`;
    shape.style.height = `${Math.max(48, item.h * 36)}px`;
    shape.style.borderColor = item.color;
    shape.innerHTML = `<span class="pending-item-icon">${item.icon}</span><strong>${item.name}</strong>`;
    if (!placed) this.attachPendingItem(shape, item);
    panel.appendChild(shape);

    const rarity = item.rarity ? RARITY_INFO[item.rarity].name : '普通';
    const details = document.createElement('div');
    details.className = 'pending-item-details';
    const description = item.description
      || CONSUMABLE_INFO[item.kind as ConsumableKind]?.desc
      || (item.kind === 'ammo' ? `备用弹药 ×${item.stack || 0}` : '可放入背包的物品');
    details.innerHTML =
      `<div><span>品质</span><strong style="color:${item.color}">${rarity}</strong></div>` +
      `<div><span>尺寸</span><strong>${item.w} × ${item.h}</strong></div>` +
      `<div><span>价值</span><strong>${item.value || 0}</strong></div>` +
      `<p>${description}</p>`;
    panel.appendChild(details);

    const status = document.createElement('div');
    status.className = `pending-placement-status ${placed ? 'ready' : ''}`;
    status.textContent = placed ? '已放入背包' : '拖入左侧空位后可确认';
    panel.appendChild(status);

    const list = document.getElementById('pending-discard-list')!;
    list.innerHTML = '';
    if (session.discards.length === 0) {
      const empty = document.createElement('span');
      empty.className = 'pending-discard-empty';
      empty.textContent = '暂无暂存物品';
      list.appendChild(empty);
    } else {
      for (const record of session.discards) {
        const chip = document.createElement('div');
        chip.className = 'pending-discard-chip';
        chip.style.borderColor = record.item.color;
        chip.textContent = `${record.item.icon} ${record.item.name}  ${record.item.w}×${record.item.h}`;
        list.appendChild(chip);
      }
    }

    (document.getElementById('pending-confirm') as HTMLButtonElement).disabled = !placed;
    (document.getElementById('pending-undo') as HTMLButtonElement).disabled = session.discards.length === 0;
  }

  private stagePendingDiscard(item: InvItem): void {
    if (!this.bag || !this.pendingLoot || item === this.pendingLoot.item || !this.bag.items.includes(item)) return;
    const snapshot = this.bag.createSnapshot();
    this.bag.removeItem(item);
    this.pendingLoot.discards.push({ item, snapshot });
    this.closePopup();
    this.renderInventory();
  }

  private undoPendingDiscard(): void {
    if (!this.bag || !this.pendingLoot) return;
    const record = this.pendingLoot.discards.pop();
    if (!record) return;
    this.cancelActiveDrag(false);
    this.closePopup();
    this.bag.restoreSnapshot(record.snapshot);
    this.renderInventory();
  }

  private confirmPendingLoot(): void {
    if (!this.bag || !this.pendingLoot) return;
    if (!this.bag.items.includes(this.pendingLoot.item)) {
      this.showToast('请先把新物品放入背包');
      return;
    }
    this.cancelActiveDrag(false);
    const session = this.pendingLoot;
    const discarded = session.discards.map((record) => record.item);
    this.finishPendingLoot();
    session.onConfirm(discarded);
  }

  private abandonPendingLoot(): void {
    if (!this.bag || !this.pendingLoot) return;
    this.cancelActiveDrag(false);
    const session = this.pendingLoot;
    this.bag.restoreSnapshot(session.initialSnapshot);
    this.finishPendingLoot();
    session.onAbandon();
  }

  private finishPendingLoot(): void {
    this.pendingLoot = null;
    this.closePopup();
    this.bagOverlay.classList.remove('pending-mode');
    document.getElementById('bag-title')!.textContent = '背包';
    this.bagOverlay.style.display = 'none';
    if (this.onBagToggle) this.onBagToggle(false);
  }
  ...
  private showItemPopup(item: InvItem, x: number, y: number): void {
    ...
    if (item.w !== item.h) {
      addBtn('↻ 旋转', () => {
        const rotated = this.bag!.rotateItem(item);
        this.showToast(rotated ? '物品已旋转' : '当前位置没有旋转空间');
      });
    }
    if (this.pendingLoot) {
      if (item === this.pendingLoot.item) {
        addBtn('移回新发现区', () => { this.bag!.removeItem(item); });
      } else {
        addBtn('移入丢弃暂存区', () => this.stagePendingDiscard(item), true);
      }
    ...
    if (!this.pendingLoot) addBtn('丢弃', () => { this.bag!.removeItem(item); }, true);
  ...
  private renderRouteChoicePreview(snapshot: RouteMapSnapshot): void {
    ...
    const nodeEls = nodes.filter((node) => visibleIds.has(node.id)).map((node) => {
      const typeRevealed = revealedTypeIds.has(node.id);
      const info = typeRevealed ? ROOM_INFO[node.type] : { name: '未知房间', icon: '?' };
      const cls = [
      ...
      const art = typeRevealed ? ROOM_ART[node.type] : null;
      const icon = art
      ...
      const showLabel = typeRevealed && (snapshot.currentId === node.id || choiceIds.has(node.id) ||
        node.visited || node.type === 'corridor' || node.type === 'exit' || node.type === 'boss');
      const label = showLabel
        ? `<g class="route-choice-map-label"><rect x="${(x - 21).toFixed(1)}" y="${(y + 19).toFixed(1)}" width="42" height="16" rx="7"></rect>` +
          `<text x="${x.toFixed(1)}" y="${(y + 31).toFixed(1)}">${typeRevealed ? (ROOM_LABEL[node.type] || info.name) : '未知'}</text></g>`
        : '';
  ...
  private drawRouteMap(snapshot: RouteMapSnapshot): void {
    ...
    const nodeEls = nodes.filter((node) => visibleIds.has(node.id)).map((node) => {
      ...
      const cls = [
        'route-node',
        typeRevealed ? `route-${node.type}` : 'route-unknown',
        typeRevealed && hasRoomArt(node.type) ? 'has-art' : '',
        node.visited ? 'visited' : '',
        snapshot.currentId === node.id ? 'current' : '',
        choiceIds.has(node.id) ? 'next' : '',
      ...
      const x = mapX(node.x);
      const y = mapY(node.y);
      const icon = typeRevealed ? roomArtHtml(node.type, info.icon, 'route-node-art') : '?';
      const showLabel = typeRevealed && (snapshot.currentId === node.id || choiceIds.has(node.id) || node.visited);
      const fixedLabel = typeRevealed && (node.type === 'corridor' || node.type === 'exit' || node.type === 'boss');
