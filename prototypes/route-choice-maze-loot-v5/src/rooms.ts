...
export type RoomType = 'corridor' | 'exit' | 'boss' | 'storage' | 'supply' | 'gem' | 'blessing' | 'curse' | 'safehouse' | 'shop';
...
export const ROOM_INFO: Record<string, { name: string; icon: string; hint: string }> = {
  ...
  storage:  { name: '储物间', icon: '📦', hint: '无怪物与理智消耗，可直接搜索多个箱子' },
  ...
  exit:     { name: '撤离点', icon: '🚩', hint: '抵达后可随时撤离，无需击败 BOSS' },
  boss:     { name: 'BOSS 房', icon: '👹', hint: '击败本层主宰后可在此直接撤离' },
};
...
export interface RouteMapSnapshot {
  ...
  choiceIds: string[];
  choiceDirections: Record<string, RouteDoorDirection>;
  visibilityMode: 'full' | 'fog' | 'surveyed';
  visibleIds: string[];
  revealedTypeIds: string[];
  floors: number;
  lanes: number;
  minX: number;
  maxX: number;
...
export type RouteDoorDirection = 'left' | 'center' | 'right';
...
function getRouteDoorTex(theme: Theme, type: RoomType): THREE.CanvasTexture {
  ...
  if (type === 'supply' || type === 'storage') {
    ...
    begin(); g.arc(cx, y(176), x(58), 0, Math.PI * 2); stroke();
    begin(); g.moveTo(cx, y(88)); g.lineTo(cx, y(264)); g.moveTo(x(88), y(222)); g.lineTo(cx, y(264)); g.lineTo(x(168), y(222)); stroke();
  } else if (type === 'boss') {
...
export class Dungeon {
  ...
  private routePreviousId: string | null = null;
  ...
  private readonly routeLanes: number = this.routeRoomsPerSegment * 2 + 1;
  ...
  // 每段地图先生成完整二维格点图。节点坐标唯一，边只连接上下左右相邻房间。
  ...
  routeSnapshot(): RouteMapSnapshot {
    ...
    const allNodeIds = this.routeNodes.map((n) => n.id);
    const current = this.routeNodes.find((n) => n.id === this.routeCurrentId);
    const choiceDirections: Record<string, RouteDoorDirection> = {};
    if (current) {
      for (const choice of this.routeDoorChoicesFor(current)) {
        choiceDirections[choice.node.id] = choice.direction;
      }
    }
    return {
      ...
      revealedTypeIds: allNodeIds,
      floors: this.routeFloors,
      lanes: this.routeLanes,
      minX: Math.min(...this.routeNodes.map((n) => n.x)),
      maxX: Math.max(...this.routeNodes.map((n) => n.x)),
      minY: Math.min(...this.routeNodes.map((n) => n.y)),
      maxY: Math.max(...this.routeNodes.map((n) => n.y)),
  ...
  appendEntrance(): Room {
    ...
    return this.append('corridor', false, start ? this.routeDoorChoicesFor(start) : []);
  }
  ...
  appendRouteNode(node: RouteNode, matchNodeDepth: boolean = false): Room {
    ...
    const revisited = live.visited;
    this.depthCount = Math.max(0, live.depth);
    this.routePreviousId = this.routeCurrentId;
    live.visited = true;
    this.routeCurrentId = live.id;
    const routeDoors = this.routeDoorChoicesFor(live);
    return this.append(live.type, false, routeDoors, live, revisited, this.routeEntryOrientation(live));
  }

  markSanityPaid(room: Room): void {
    const node = this.routeNodes.find((n) => n.id === room.routeNodeId);
    if (node) node.sanityPaid = true;
  }

  markRoomCleared(room: Room): void {
    room.cleared = true;
    const node = this.routeNodes.find((n) => n.id === room.routeNodeId);
    if (node) node.cleared = true;
  }

  markRoomResolved(room: Room): void {
    const node = this.routeNodes.find((n) => n.id === room.routeNodeId);
    if (node) node.resolved = true;
  }

  markPickupTaken(room: Room): void {
    const node = this.routeNodes.find((n) => n.id === room.routeNodeId);
    if (node) node.pickupTaken = true;
  }

  markCrateSearch(room: Room, crateIndex: number, searchCount: number): void {
    const node = this.routeNodes.find((n) => n.id === room.routeNodeId);
    if (!node) return;
    if (!node.crateSearchCounts) node.crateSearchCounts = [];
    node.crateSearchCounts[crateIndex] = searchCount;
  }

  roomNodeState(room: Room): RouteNode | null {
    return this.routeNodes.find((n) => n.id === room.routeNodeId) || null;
  }
  ...
  private routeDoorChoicesFor(current: RouteNode): RouteDoorChoice[] {
    ...
    if (targets.length === 1) return [{ direction: 'center', node: targets[0] }];
    const facing = this.routeFacingVector(current);
    const signedAngle = (target: RouteNode): number => {
      const tx = target.x - current.x;
      const ty = target.y - current.y;
      return Math.atan2(facing.x * ty - facing.y * tx, facing.x * tx + facing.y * ty);
    };
    if (targets.length === 2) {
      const byForwardness = targets.slice().sort((a, b) => Math.abs(signedAngle(a)) - Math.abs(signedAngle(b)));
      const center = byForwardness[0];
      const side = byForwardness[1];
      const sideDirection: RouteDoorDirection = signedAngle(side) >= 0 ? 'left' : 'right';
      const choices: RouteDoorChoice[] = [
        { direction: 'center', node: center },
        { direction: sideDirection, node: side },
      ];
      return choices.sort((a, b) => this.routeDirectionRank(a.direction) - this.routeDirectionRank(b.direction));
    }

    const ordered = targets.slice().sort((a, b) => signedAngle(b) - signedAngle(a));
    return [
      { direction: 'left', node: ordered[0] },
      { direction: 'center', node: ordered[1] },
      { direction: 'right', node: ordered[2] },
    ];
  }

  private routeDirectionRank(direction: RouteDoorDirection): number {
    return direction === 'left' ? 0 : direction === 'center' ? 1 : 2;
  }

  private routeFacingVector(current: RouteNode): { x: number; y: number } {
    const previous = this.routeNodes.find((n) => n.id === this.routePreviousId);
    if (!previous || previous.id === current.id) return { x: 0, y: 1 };
  ...
  private routeEntryOrientation(current: RouteNode): number {
    const facing = this.routeFacingVector(current);
    const quarter = Math.round(Math.atan2(facing.y, facing.x) / (Math.PI / 2));
    return (quarter + 4) % 4;
  }
  ...
  private generateRouteMap(): void {
    ...
    let accepted: RouteNode[] | null = null;
    for (let i = 0; i < 120; i++) {
      const candidate = this.buildMazeRouteCandidate();
      if (!candidate) continue;
      if (this.mazeCandidateValid(candidate)) {
        accepted = candidate;
        break;
      }
    }

    this.routeNodes = accepted || this.buildMazeFallback();
    const start = this.routeNodes.find((n) => n.type === 'corridor' && n.floor === 1);
    this.routeCurrentId = start ? start.id : null;
  }

  // 固定三个关键房间的二维坐标，其余房间随机落点；用无向边形成可回头、可绕圈的迷宫。
  // 这里只校验图结构，不再估算每条路线的期望收益。
  private buildMazeRouteCandidate(): RouteNode[] | null {
    const nodes: RouteNode[] = [];
    const segment = this.routeSegmentIndex;
    const addNode = (id: string, x: number, y: number, type: RoomType, visited: boolean = false): RouteNode => {
      const node: RouteNode = {
        id, type, floor: 1, lane: x + 6, x, y,
        depth: this.routeSegmentStartDepth + 1,
        links: [], visited, cleared: false, sanityPaid: false, resolved: false,
        pickupTaken: false, crateSearchCounts: [],
        layoutSeed: Math.floor(Math.random() * 0x7fffffff),
        risk: this.routeRisk(type), reward: this.routeReward(type),
      };
      nodes.push(node);
      return node;
    };

    const start = addNode(`r${segment}-start`, -5, -3, 'corridor', true);
    const exit = addNode(`r${segment}-exit`, 5, -3, 'exit');
    const boss = addNode(`r${segment}-boss`, 5, 3, 'boss');

    const minNodeDistance = 1.75;
    const coords: Array<[number, number]> = [];
    for (let y = -4; y <= 4; y += 2) {
      for (let x = -4; x <= 4; x += 2) {
        const clearOfFixed = [start, exit, boss].every((node) => Math.hypot(x - node.x, y - node.y) >= minNodeDistance);
        if (clearOfFixed) coords.push([x, y]);
      }
    }
    ...
    if (coords.length < 18) return null;
    const fixedTypes: RoomType[] = ['safehouse', 'shop', 'storage', 'blessing', 'curse'];
    for (let i = 0; i < 18; i++) {
      const [baseX, baseY] = coords[i];
      const x = baseX + (Math.random() - 0.5) * 0.14;
      const y = baseY + (Math.random() - 0.5) * 0.14;
      if (nodes.some((node) => Math.hypot(x - node.x, y - node.y) < minNodeDistance)) return null;
      const approxDepth = this.routeSegmentStartDepth + 2 + Math.floor(Math.random() * 9);
      const type = fixedTypes[i] || this.rollRouteType(2 + (i % 8), approxDepth);
      addNode(`r${segment}-n${i}`, x, y, type);
    }

    const degree = new Map<string, number>(nodes.map((n) => [n.id, 0]));
    const link = (a: RouteNode, b: RouteNode): boolean => {
      if (a.id === b.id || a.links.includes(b.id)) return false;
      if ((degree.get(a.id) || 0) >= 3 || (degree.get(b.id) || 0) >= 3) return false;
      if (this.mazeEdgeBlockedByNode(a, b, nodes)) return false;
      a.links.push(b.id);
      b.links.push(a.id);
    ...
    const distance = (a: RouteNode, b: RouteNode): number => Math.hypot(a.x - b.x, a.y - b.y);

    // 随机 Prim 骨架保证全图连通，同时限制每个物理房最多三扇路线门。
    const connected = new Set<string>([start.id]);
    while (connected.size < nodes.length) {
      let best: { a: RouteNode; b: RouteNode; score: number } | null = null;
      for (const a of nodes) {
        if (!connected.has(a.id) || (degree.get(a.id) || 0) >= 3) continue;
        for (const b of nodes) {
          if (connected.has(b.id) || (degree.get(b.id) || 0) >= 3) continue;
          const score = distance(a, b) + Math.random() * 2.4;
          if (!best || score < best.score) best = { a, b, score };
        }
      }
      if (!best || !link(best.a, best.b)) return null;
      connected.add(best.b.id);
    }

    // 添加少量近邻边制造环路。玩家可绕圈，但节点度数上限避免门过密。
    const pairs: Array<{ a: RouteNode; b: RouteNode; score: number }> = [];
    for (let i = 0; i < nodes.length; i++) {
      for (let j = i + 1; j < nodes.length; j++) {
        const a = nodes[i], b = nodes[j];
        if (a.links.includes(b.id)) continue;
        pairs.push({ a, b, score: distance(a, b) + Math.random() * 3.2 });
      }
    }
    pairs.sort((a, b) => a.score - b.score);
    let extraEdges = 0;
    for (const pair of pairs) {
      if (extraEdges >= 7) break;
      if (link(pair.a, pair.b)) extraEdges += 1;
    }

    const distances = this.mazeDistancesFrom(start.id, nodes);
    if (distances.size !== nodes.length) return null;
    for (const node of nodes) {
      const steps = distances.get(node.id) || 0;
      node.floor = steps + 1;
      node.depth = this.routeSegmentStartDepth + Math.max(1, steps);
      node.lane = node.x + 6;
      node.links.sort();
    }
    start.floor = 1;
    start.depth = this.routeSegmentStartDepth;
    this.enforceSpecialRoomDistance(nodes, distances);
    exit.risk = '安全撤离';
    boss.risk = '首领战';
    return nodes;
  }

  private mazeDistancesFrom(startId: string, nodes: RouteNode[]): Map<string, number> {
    ...
    while (queue.length > 0) {
      ...
      for (const nextId of node.links) {
        ...
        result.set(nextId, (result.get(id) || 0) + 1);
        queue.push(nextId);
      }
    }
    return result;
  ...
  private enforceSpecialRoomDistance(nodes: RouteNode[], distances: Map<string, number>): void {
    const isSpecial = (node: RouteNode): boolean => node.type === 'shop' || node.type === 'safehouse';
    for (const node of nodes) {
      if (isSpecial(node) && (distances.get(node.id) || 0) <= 3) node.type = 'storage';
    }

    const farCandidates = nodes
      .filter((node) =>
        (distances.get(node.id) || 0) >= 4 &&
        node.type !== 'corridor' && node.type !== 'exit' && node.type !== 'boss'
      )
      .sort((a, b) => (distances.get(b.id) || 0) - (distances.get(a.id) || 0));
    for (const requiredType of ['safehouse', 'shop'] as RoomType[]) {
      if (nodes.some((node) => node.type === requiredType)) continue;
      const replacement = farCandidates.find((node) => node.type !== 'safehouse' && node.type !== 'shop');
      if (replacement) replacement.type = requiredType;
    }

    for (const node of nodes) {
      node.risk = this.routeRisk(node.type);
      node.reward = this.routeReward(node.type);
    }
  }

  private mazeShortestPath(fromId: string, toId: string, nodes: RouteNode[]): string[] {
    const byId = new Map(nodes.map((n) => [n.id, n]));
  ...
  private mazeEdgeBlockedByNode(a: RouteNode, b: RouteNode, nodes: RouteNode[]): boolean {
    const clearance = 0.82;
    const dx = b.x - a.x;
    const dy = b.y - a.y;
    ...
    for (const node of nodes) {
      ...
      const projection = ((node.x - a.x) * dx + (node.y - a.y) * dy) / lengthSq;
      if (projection <= 0 || projection >= 1) continue;
      const t = Math.max(0, Math.min(1, projection));
      const closestX = a.x + dx * t;
      const closestY = a.y + dy * t;
      if (Math.hypot(node.x - closestX, node.y - closestY) < clearance) return true;
    }
    return false;
  }

  private mazeCandidateValid(nodes: RouteNode[]): boolean {
    const start = nodes.find((n) => n.type === 'corridor');
    const exit = nodes.find((n) => n.type === 'exit');
    const boss = nodes.find((n) => n.type === 'boss');
    if (!start || !exit || !boss) return false;
    const distances = this.mazeDistancesFrom(start.id, nodes);
    if (!nodes.some((node) => node.type === 'shop') || !nodes.some((node) => node.type === 'safehouse')) return false;
    if (nodes.some((node) =>
      (node.type === 'shop' || node.type === 'safehouse') && (distances.get(node.id) || 0) <= 3
    )) return false;
    for (let i = 0; i < nodes.length; i++) {
      if (nodes[i].links.length > 3) return false;
      for (let j = i + 1; j < nodes.length; j++) {
        if (Math.hypot(nodes[i].x - nodes[j].x, nodes[i].y - nodes[j].y) < 1.75) return false;
      }
      for (const linkedId of nodes[i].links) {
        const target = nodes.find((node) => node.id === linkedId);
        if (target && nodes[i].id < target.id && this.mazeEdgeBlockedByNode(nodes[i], target, nodes)) return false;
      }
    }
    if (start.links.length < 2 || exit.links.length < 2 || boss.links.length < 2) return false;
    const edgeCount = nodes.reduce((sum, n) => sum + n.links.length, 0) / 2;
    if (edgeCount - nodes.length + 1 < 4) return false;

    const toBoss = this.mazeShortestPath(start.id, boss.id, nodes);
    const toExit = this.mazeShortestPath(start.id, exit.id, nodes);
    const bossToExit = this.mazeShortestPath(boss.id, exit.id, nodes);
    if (toBoss.length < 6 || toExit.length < 6 || bossToExit.length < 4) return false;
    const bossInternal = new Set(toBoss.slice(1, -1));
    const repeated = toExit.slice(1, -1).filter((id) => bossInternal.has(id)).length;
    return repeated <= 3;
  }

  private buildMazeFallback(): RouteNode[] {
    for (let i = 0; i < 2000; i++) {
      const candidate = this.buildMazeRouteCandidate();
      if (candidate && this.mazeCandidateValid(candidate)) return candidate;
    }
    throw new Error('无法生成满足关键路径重复节点约束的随机迷宫');
  }

  startNextSegment(): void {
    this.routeNodes = [];
    this.routeCurrentId = null;
    this.routePreviousId = null;
    this.generateRouteMap();
  }

  isBossDefeated(): boolean {
    const boss = this.routeNodes.find((n) => n.type === 'boss');
    return !!boss?.cleared;
  }

  private buildPhysicalRouteCandidate(): RouteMapCandidate {
    const nodes: RouteNode[] = [];
    const byCoord = new Map<string, RouteNode>();
    const coordKey = (x: number, y: number): string => `${x},${y}`;
    const addNode = (id: string, floor: number, x: number, y: number, type: RoomType, visited: boolean = false): RouteNode => {
      const depth = this.routeDepthForFloor(floor);
      const node: RouteNode = {
        id,
        type,
        floor,
        lane: x + this.routeRoomsPerSegment,
        x,
        y,
        depth,
        links: [],
      ...
      nodes.push(node);
      byCoord.set(coordKey(x, y), node);
      return node;
    };

    const segment = this.routeSegmentIndex;
    const start = addNode(`r${segment}-start`, 1, 0, 0, 'corridor', true);
    for (let distance = 1; distance <= this.routeRoomsPerSegment; distance++) {
      const floor = distance + 1;
      for (let x = -distance; x <= distance; x++) {
        const y = distance - Math.abs(x);
        let type: RoomType;
        if (floor === this.routeFloors) type = 'boss';
        else if (floor === this.routeFloors - 1) type = 'shop';
        else if (floor === this.routeRestFloor) type = 'safehouse';
        else type = this.rollRouteType(floor, this.routeDepthForFloor(floor));
        addNode(`r${segment}-${x}-${y}`, floor, x, y, type);
      }
    }

    for (const node of nodes) {
      if (node.floor >= this.routeFloors) continue;
      const targets: Array<[number, number]> = [[node.x, node.y + 1]];
      if (node.x <= 0) targets.push([node.x - 1, node.y]);
      if (node.x >= 0) targets.push([node.x + 1, node.y]);
      for (const [x, y] of targets) {
        const target = byCoord.get(coordKey(x, y));
        if (target && !node.links.includes(target.id)) node.links.push(target.id);
      }
    }

    if (start.links.length !== 3) return { nodes, fairness: Number.POSITIVE_INFINITY, routeCount: 0 };
  ...
  private rollRouteFloorTypes(floor: number): RoomType[] {
    ...
    const fallback: RoomType[] = floor <= 3
      ? ['storage', 'supply', 'gem']
      : ['storage', 'gem', depth >= 6 ? 'shop' : 'blessing'];
  ...
  private rollRouteType(floor: number, depth: number): RoomType {
    const weights: { type: RoomType; weight: number }[] = [
      { type: 'storage', weight: floor <= 3 ? 24 : 18 },
      { type: 'supply', weight: floor <= 3 ? 30 : 24 },
  ...
  private enumerateRouteScores(nodes: RouteNode[]): number[] {
    ...
    if (!start || !nodes.some((n) => n.type === 'boss')) return [];
  ...
  private routeRisk(type: RoomType): string {
    ...
    if (type === 'exit') return '安全节点';
    if (type === 'boss') return '首领战';
    if (type === 'storage') return '无战斗';
  ...
  private routeReward(type: RoomType): string {
    ...
    if (type === 'exit') return '安全带出战利品';
    if (type === 'boss') return '圣物/直接撤离';
    if (type === 'storage') return '多箱物资';
  ...
  append(
    type: RoomType,
    countDepth: boolean = true,
    routeDoors: RouteDoorChoice[] = [],
    nodeState: RouteNode | null = null,
    revisited: boolean = false,
    entryOrientation: number = 1
  ): Room {
    if (countDepth) this.depthCount += 1;
    if (type === 'shop') this.lastShopDepth = this.depthCount;  // 记录商店深度（控制间隔）
    // 纵深加大（前后空间更大，左右宽度不变）
    const length = type === 'shop' ? 16 : type === 'curse' ? 16 : type === 'safehouse' || type === 'exit' ? 18 : type === 'boss' ? 34 : 24;
    const zEntry = this.nextZ;
    const zExit = zEntry - length;
    this.nextZ = zExit;

    const group = new THREE.Group();
    let layoutState = nodeState?.layoutSeed || 1;
    const layoutRandom = (): number => {
      layoutState = (layoutState * 1664525 + 1013904223) >>> 0;
      return layoutState / 0x100000000;
    };
    const layoutMirror = entryOrientation >= 2 ? -1 : 1;
    const reverseDepthOrder = entryOrientation === 1 || entryOrientation === 2;
    ...
    const torchSpots: number[] = type === 'corridor' ? [] : [0.24, 0.72];
    ...
    const addPickup = (obj: THREE.Mesh, kind: string, color: number, hx: number, hz: number): void => {
      ...
      obj.userData.taken = !!nodeState?.pickupTaken;
      obj.userData.active = !nodeState?.pickupTaken;   // 是否可点击（祝福需清场后才激活）
    ...
    const rollCrateLoot = (): InvItem => {
      if (type === 'gem') return rollGemContent(this.depthCount);
      if (type === 'storage') return Math.random() < 0.55
        ? rollSupplyContent(this.depthCount)
        : rollGemContent(this.depthCount);
      return rollSupplyContent(this.depthCount);
    };
    const rollCrateCount = (): number => {
      if (type === 'supply' || type === 'gem') return 2 + Math.floor(Math.random() * 3);
      if (type === 'storage') return 2 + Math.floor(Math.random() * 2);
      if (type === 'safehouse') return Math.floor(Math.random() * 2);
      return 0;
    };
    const makeCrateLoots = (): InvItem[][] => Array.from({ length: rollCrateCount() }, () =>
      Array.from({ length: 2 + Math.floor(Math.random() * 2) }, () => rollCrateLoot())
    );
    if (nodeState && !nodeState.crateLoots) {
      nodeState.crateLoots = makeCrateLoots();
      nodeState.crateSearchCounts = new Array(nodeState.crateLoots.length).fill(0);
    }
    const crateLoots = nodeState?.crateLoots || makeCrateLoots();
    const crateCount = crateLoots.length;
    const crateGeo = new THREE.BoxGeometry(1.1, 0.9, 0.85);
    for (let i = 0; i < crateCount; i++) {
      const contents = crateLoots[i];
      const searchCount = Math.min(contents.length, nodeState?.crateSearchCounts?.[i] || 0);
      const searched = searchCount >= contents.length;
      const crate = new THREE.Mesh(crateGeo, new THREE.MeshStandardMaterial({
        color: searched ? 0x3a3a3a : 0x7a5a30,
        emissive: searched ? new THREE.Color(0x000000) : new THREE.Color(0xffffff),
        emissiveIntensity: 0,
      }));
      const rowCount = Math.ceil(crateCount / 2);
      const row = Math.floor(i / 2);
      const visualRow = reverseDepthOrder ? rowCount - 1 - row : row;
      const side = i % 2 === 0 ? -1 : 1;
      crate.position.set(side * layoutMirror * 1.65, 0.45, zMid + 2.5 - visualRow * 5);
      crate.rotation.y = (layoutRandom() * 0.5 - 0.25) * layoutMirror;
      crate.castShadow = true;
      crate.userData.crate = true;
      crate.userData.searched = searched;
      crate.userData.searchCount = searchCount;
      crate.userData.routeCrateIndex = i;
      crate.userData.contents = contents;
      crate.userData.content = contents[searchCount];
      group.add(crate);
      // 箱子高亮统一为白色；物品品质色只用于搜索进度环和拾取反馈。
      const halo = new THREE.Mesh(
    ...
    if (type === 'storage') {
      const shelfMat = new THREE.MeshStandardMaterial({ color: 0x4f4437, roughness: 0.95 });
      for (const side of [-1, 1]) {
        const shelf = new THREE.Group();
        for (let level = 0; level < 3; level++) {
          const plank = new THREE.Mesh(new THREE.BoxGeometry(2.7, 0.12, 0.72), shelfMat);
          plank.position.set(0, 0.45 + level * 0.72, 0);
          shelf.add(plank);
        }
        for (const x of [-1.25, 1.25]) {
          const post = new THREE.Mesh(new THREE.BoxGeometry(0.12, 2.1, 0.12), shelfMat);
          post.position.set(x, 1.05, 0);
          shelf.add(post);
        }
        shelf.position.set(side * layoutMirror * 3.35, 0, zMid + (side < 0 ? 4.6 : -4.6));
        shelf.rotation.y = side < 0 ? 0.18 : -0.18;
        group.add(shelf);
      }
    } else if (type === 'curse') {
      // 黑暗祭坛 + 可点击宝珠（点击抉择）
      ...
      const gate = new THREE.Mesh(
        new THREE.TorusGeometry(1.25, 0.16, 12, 32),
        new THREE.MeshStandardMaterial({ color: 0x78ffb0, emissive: 0x17663c, emissiveIntensity: 1.4 })
      );
      gate.position.set(0, 1.65, zMid);
      group.add(gate);
      addPickup(gate, 'exit', 0x67ffae, 0, zMid);
      const beacon = new THREE.PointLight(0x67ffae, 9, 18, 1.5);
      beacon.position.set(0, 2.2, zMid);
      group.add(beacon);
    } else if (type === 'boss') {
      // 王座 + 双红炬
    ...
    if (type === 'supply' || type === 'gem') {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        bone.position.set(
          (layoutRandom() - 0.5) * (w - 3) * layoutMirror, 0.06,
          zEntry - 2 - layoutRandom() * (length - 4)
        );
        bone.rotation.set(Math.PI / 2, 0, layoutRandom() * Math.PI * layoutMirror);
        group.add(bone);
      }
    }
    ...
    const room: Room = {
      ...
      crates,
      pickups,
      searched: crates.filter((crate) => crate.userData.searched).length,
      torches,
      cleared: !!nodeState?.cleared,
      doorPanels,
      doorProgress: 0,
      doorRumbled: false,
      doorW,
      routeDoors: routeDoorVisuals,
      routeNodeId: nodeState?.id || null,
      revisited,
    };
